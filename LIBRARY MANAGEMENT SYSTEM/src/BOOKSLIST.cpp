...
void BOOKSLIST::readFile () {
    ifstream inFile;
    inFile.open("Data\\Data Book.txt", ios::in);
    while (!inFile.eof()) {
        BOOK temp;
...
void BOOKSLIST::exportFIle () {
    ofstream outFile;
    outFile.open("Data\\Data Book.txt", ios::out);
...
void BOOKSLIST::deleteBook(string idBook, BORROW_RETURNSLIST &BRsList) {
	bool Check = false;
	for (int i = 0; i < booksList.size(); i++) {
		if (booksList[i].getIdBook() == idBook) {
            for (int j = 0; j < BRsList.getBRsList().size(); j++) {
                if (BRsList.getBRsList()[j].getStatus() == 1) {
                    for (int k = 0; k < BRsList.getBRsList[j].listBooksToBorrow.getListBooksToBorrow().size(); k++) {
                        if (BRsList.getBRsList[j].listBooksToBorrow.getListBooksToBorrow()[k] == booksList[i].getTitle()) {
                            BRsList.getBRsList[j].listBooksToBorrow.getListBooksToBorrow().erase(BRsList.getBRsList[j].listBooksToBorrow.getListBooksToBorrow().begin() + k);
                            break;
                        }
                    }
                    if (BRsList.getBRsList[j].listBooksToBorrow.getListBooksToBorrow().size() == 0) {
                        BRsList.getBRsList()[j].setStatus(0);
                    }
                }
            }
            booksList.erase(booksList.begin() + i);
